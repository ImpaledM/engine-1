...
class sections {
	...
	function __construct($all=false) {
		...
		if (isset($_GET ['section'])) $this->get_param();
	}
	function get_param() {
	...
	function update_plain() {
		$res = $this->db->query ( 'SELECT * FROM `section` WHERE `sys` IS NULL ORDER BY `priority`' );
		while ( $row = $this->db->fetch ( $res ) ) {
	...
	function get_sections($all) {
		...
		$res = $this->db->query ( 'SELECT a.*, b.param FROM `section` AS a, `section_present` AS b WHERE a.id=b.id1 AND a.id=b.id2'.$add_query.' ORDER BY `priority`');
		while ( $row = $this->db->fetch ( $res ) ) {
